...
class TestBuildJourney:
    def _issue(self, project="BAC", state="Submitted", created=0):
        return {
            "idReadable": "BAC-1",
    ...
    def test_project_move(self):
        ...
        events = _build_journey(self._issue(project="BAC", created=0), acts, now_ms=10 * DAY_MS)
        assert events[0]["dept"] == "Backend"
    ...
    def test_initial_state_uses_state_hint(self):
        ...
        events = _build_journey(
            self._issue(project="BAC", state="Dev QA", created=0),
            [], now_ms=5 * DAY_MS,
        )
